...
strategy = RawClassification(bars, event_queue, RandomForestClassifier,
                             processor=ClassificationData(bars, 14, 2), reoptimize_days=30)
port = PercentagePortFolio(bars, event_queue, order_queue,
                           percentage=0.05, rebalance=BaseRebalance(event_queue))
